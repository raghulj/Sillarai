            ...
            if ($this.is('button')) {
                ...
                if (($.isFunction(options.save) && options.save.call(self, event, hash)) !== false || !options.save) {
                    self.value(hash.value);
			                    postExpense(this.id,hash.value );
                }

            } else if ($this.is(self[0].tagName) || $this.hasClass('inlineEdit-placeholder')) {
                self
                    .html('<input type="text" onfocus="javascript:showhelp();" onblur="javascript:hidehelp();" size="51" value="'+ self.value() +'"> <button style="color:#fff;background:#115805">'+ options.buttonText +'</button>')
                    .find('input')
                        .bind('blur', function() {
                            if (self.timer) {
